...
// We replace the ipv4_lpm table by 2 new tables, ecmp_group and
// ecmp_nhop. ecmp_group matches on the destination IP address and uses
// modify_field_with_hash_based_offset to randomly select a valid next
// hop index. ecmp_nhop stores the necessary information for each next hop
// index (IP address, interface); this information used to be in the ipv4_lpm
// table.
...
control ingress {
    if(valid(ipv4) and ipv4.ttl > 0) {
        ...
        // SOLUTION --->
        // ipv4_lpm replaced by our 2 new tables
        apply(ecmp_group);
        apply(ecmp_nhop);
        // <--- SOLUTION
        apply(forward);
